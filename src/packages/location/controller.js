...
{
  const [error, result] = await to(service.index2(req.params.id))
  return handleResponse(error, result, req, res)
}
async function filterPerformer(req,res)
{
  const [error, result] = await to(service.index3(req.params.id))
  return handleResponse(error, result, req, res)
}
